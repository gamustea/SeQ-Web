    ...
    <div v-if="show" class="modal-overlay" @click.self="close">
      <div class="modal">
          <div class="modal-header">
            <h3>Añadir a carpeta</h3>
            <button class="close-btn" @click="close">&times;</button>
          </div>
          <div class="modal-body">
            <p class="hint">{{ scanCount }} escaneo(s) seleccionado(s)</p>
            <form @submit.prevent="submit">
              ...
              <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary" :disabled="submitting || !selectedFolderId">
                  <span v-if="submitting">Añadiendo…</span>
                  <span v-else>Añadir a carpeta</span>
                </button>
              </div>
            </form>
          </div>
...
const selectableFolders = computed(() =>
  props.folders.filter(f => f.id !== null)
)
...
function submit() {
  ...
  emit('add', Number(selectedFolderId.value))
}
</script>
