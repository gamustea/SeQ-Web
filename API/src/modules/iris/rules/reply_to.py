...
def check_reply_to(headers: dict) -> RuleResult:
    """Detect when ``Reply-To`` points to a different domain than ``From``.

    Phishing campaigns routinely set Reply-To to an attacker-controlled
    address so that replies bypass the victim's mailbox.

    Returns:
        - ``pass`` (score +3) when Reply-To is absent or matches From.
        - ``fail`` (score -10) when the domains differ.
