...
class AegisDocument(Base):
    ...
    # NUEVO: distingue entre el formato legacy (.md) y el estructurado (.json)
...
class AegisTip(Base):
    ...
    links_json = Column(JSONB,        nullable=True)   # [{text, url}, ...]
...
class AegisDocumentAlert(Base):
    ...
    position        = Column(SmallInteger, nullable=False)
    source          = Column(String(16),   nullable=False)   # 'incibe' | 'circl'
    source_label    = Column(String(32),   nullable=False)   # 'INCIBE-CERT' | 'NVD/CVE'
    title           = Column(String(256),  nullable=False)
    published       = Column(Date,         nullable=True)
    severity        = Column(String(16),   nullable=True)
    affected_brands = Column(ARRAY(String), nullable=True)   # array nativo PG
