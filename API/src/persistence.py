...
def initialize_engine(database_url: str = DEFAULT_DATABASE_URL):
    ...
    if _ENGINE is None:
        ...
        # scoped_session automáticamente crea sesiones por thread
        _SESSION_FACTORY = scoped_session(
            sessionmaker(
                bind=_ENGINE,
                expire_on_commit=False,  # CRÍTICO: Evita lazy loading después de commit
                autoflush=True,
                autocommit=False
            )
        )
...
class ScanDBManager(DBManager):
    ...
    def set_scan_as_finished(self, scan: Scan) -> bool:
        ...
        Args:
            scan: Objeto Scan a marcar como finalizado
        ...
        try:            
            ...
            self.logger.info(f"Escaneo {scan.id} marcado como finalizado exitosamente")
            return True
...
class NiktoDBManager(ScanDBManager):
    ...
    def nikto_incident_exists_with_desc(self, incident_desc: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Verificación de existencia del incidente Nikto con descripción '{incident_desc}': {exists}")
            return exists
        except SQLAlchemyError as err:
            self.logger.error(f"Error al verificar existencia del incidente Nikto: {err}")
            raise
    ...
    def get_nikto_incident_by_description(self, incident_description: str) -> Optional[NiktoIncident]:
        ...
        try:
            ...
            self.logger.info(f"Se obtuvo el incidente Nikto con descripción '{incident_description}'.")
            return incident
        except SQLAlchemyError as err:
            self.logger.error(f"Error al obtener incidente Nikto por descripción: {err}")
            raise
    ...
    def get_or_create_nikto_incident(self, incident: NiktoIncident) -> NiktoIncident:
        ...
        try:
            ...
            if new_incident:
                self.logger.info(f"Incidente '{incident.description}' ya existe, reutilizando.")
                return new_incident
